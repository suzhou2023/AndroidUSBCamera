...
class MultiCameraClient(ctx: Context, callback: IDeviceConnectCallBack?) {
    ...
    abstract class ICamera(val ctx: Context, val device: UsbDevice) : Handler.Callback,
        ...
        override fun handleMessage(msg: Message): Boolean {
            when (msg.what) {
                ...
                MSG_CAPTURE_VIDEO_PAUSE -> {
                    captureVideoPauseInternal()
                }

                MSG_CAPTURE_VIDEO_RESUME -> {
                    captureVideoResumeInternal()
                }
         ...
         * Capture video pause
         */
        fun captureVideoPause() {
            mCameraHandler?.obtainMessage(MSG_CAPTURE_VIDEO_PAUSE)?.sendToTarget()
        }

        /**
         * Capture video resume
         */
        fun captureVideoResume() {
            mCameraHandler?.obtainMessage(MSG_CAPTURE_VIDEO_RESUME)?.sendToTarget()
        }
        ...
        private fun captureVideoPauseInternal() {
            (mVideoProcess as? H264EncodeProcessor)?.apply {
                if (mVideoProcess?.isEncoding() == true) {
                    mVideoProcess?.pauseEncode()
                }
            }
            Logger.i(TAG, "capturing video pause")
        }

        private fun captureVideoResumeInternal() {
            captureStreamStopInternal()
            mMediaMuxer?.release()
            mMediaMuxer = null
    ...
    companion object {
        ...
        private const val MSG_CAPTURE_VIDEO_PAUSE = 0x05
        private const val MSG_CAPTURE_VIDEO_RESUME = 0x06
        private const val MSG_CAPTURE_VIDEO_STOP = 0x07
        private const val MSG_CAPTURE_STREAM_START = 0x08
        private const val MSG_CAPTURE_STREAM_STOP = 0x09
